...
class NativeProgramSynthesizer:
    ...
    def propose(self, goal, examples=[]):
        ...
        INPUT: examples = [counterexample1, counterexample2, ...] OR [seeded_candidate1, ...]
        The examples PRUNE the search space for iterative repair (PRINCIPLE 3 FIX).
        """
        desc = goal.get("desc", "").lower()

        # LEARNING FROM EXAMPLES (Iterative Repair)
        # If we have counterexamples or seeded solutions, learn from them
        if examples:
            self.logger.info(f"Synthesizer: Learning from {len(examples)} examples")
            # Extract constraints from counterexamples to avoid same failures
            self._learn_from_examples(examples)
    ...
    def _learn_from_examples(self, examples):
        """
        Extract constraints from past solutions (successes) or failures (counterexamples).
        This informs the next synthesis attempt (PRINCIPLE 3 FIX).
        """
        for ex in examples:
            if isinstance(ex, str) and len(ex) > 0:
                # Counterexample/feedback: "Variable x not in scope"
                self.logger.debug(f"Learning constraint from: {ex}")
    ...
    def _synthesize_reduction(self, semantics, desc):
        ...
        agg_logic = self.primitives["aggregators"].get(
            agg_key, self.primitives["aggregators"]["sum"]
        )
